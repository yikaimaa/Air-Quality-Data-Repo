...
Command-line arguments:
    --pm25-lookup      : CSV with PM2.5 stations, must contain region, latitude, longitude
    --weather-latlon   : CSV with weather stations, must contain station ID, latitude, longitude
    --out              : Output CSV path
    --threshold-km     : Distance threshold (km), default 75.0
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Map weather stations to regions via nearest PM2.5 station"
    )
    parser.add_argument(
        "--pm25-lookup",
        required=True,
        help="Path to PM2.5 station-region lookup CSV (must have region, lat, lon)"
    )
    parser.add_argument(
        "--weather-latlon",
        required=True,
        help="Path to weather stations lat/lon CSV (must have station id, lat, lon)"
    )
    parser.add_argument(
        "--out",
        required=True,
        help="Output CSV path for weather station -> region lookup"
    )
    parser.add_argument(
        "--threshold-km",
        type=float,
        default=75.0,
        help="Distance threshold in kilometers (default: 75.0)"
    )
    args = parser.parse_args()

    # ---------------- load ----------------
    pm25 = pd.read_csv(args.pm25_lookup)
    wlatlon = pd.read_csv(args.weather_latlon)
    ...
    for _, r in wlatlon_clean.iterrows():
        ...
        mapped_region = pm_region_arr[j] if best_dist <= args.threshold_km else None
        mapped_pm_station = pm_station_arr[j] if (best_dist <= args.threshold_km and pm_station_col is not None) else None
        ...
        rows.append({
            ...
            "within_threshold": best_dist <= args.threshold_km
        })

    lookup = pd.DataFrame(rows)

    # ---------------- save ----------------
    lookup.to_csv(args.out, index=False)

    # ---------------- diagnostics ----------------
    print(f"Saved: {args.out}")
    print(f"Weather stations processed: {len(lookup):,}")
    print(f"Assigned (within {args.threshold_km:.0f} km): {lookup['within_threshold'].sum():,}")
