...
NEW (region mapping):
- Optionally attach a "region" (e.g., Region of Waterloo, York Region, City of Toronto) via a
  Station ID -> region lookup CSV committed in the repo.
- Default lookup path: Datasets/Ontario/PM25/station_region_lookup.csv
  Expected columns: "Station ID", "region"
  (extra columns are ignored)

Why lookup (not GIS in CI)?
- Point-in-polygon with official boundaries usually requires geopandas/GDAL (heavy & fragile in CI).
- A small curated lookup (Ontario has ~39 stations in this dataset) is lightweight and reproducible.

Outputs (defaults):
- outputs/pm25_hourly_wide_final.csv
- outputs/station_lookup.csv  (includes region if available)
- If region lookup missing, a template is written:
    outputs/station_region_lookup_TEMPLATE.csv

Recommended GitHub Actions call:
  uv run --no-project python scripts/build_hourly_pm25.py \
    --input-dir "Datasets/Ontario/PM25" \
    --station-region-lookup "Datasets/Ontario/PM25/station_region_lookup.csv" \
...
DEFAULT_STATION_REGION_LOOKUP = Path("Datasets/Ontario/PM25/station_region_lookup.csv")
...
def groupby_apply_robust(gb, func):
    ...
    - Newer pandas: apply(..., include_groups=False) avoids deprecation warning/future behavior changes
...
def attach_station_region(
    ...
    """
    Attach a 'region' column to station_lookup via a Station ID -> region CSV lookup.

    If lookup missing, write a template (Station ID, station_name, latitude, longitude, region)
    and return station_lookup with region=NaN.
    """
    out = station_lookup.copy()
    if out.empty:
    ...
    if not lookup_path.exists():
        log(f"[WARN] station region lookup not found: {lookup_path.as_posix()}")
        tmpl = out.copy()
        tmpl["region"] = ""
        template_out.parent.mkdir(parents=True, exist_ok=True)
        tmpl.to_csv(template_out, index=False)
        log(f"[INFO] wrote template: {template_out.as_posix()}")
        out["region"] = pd.NA
        if require_region:
            raise FileNotFoundError(
                f"Region lookup required but not found: {lookup_path.as_posix()}. "
                f"Fill the template at {template_out.as_posix()} and commit it to the repo."
            )
        return out

    reg = pd.read_csv(lookup_path)
    ...
    out = out.merge(reg[["Station ID", "region"]], on="Station ID", how="left")

    n_missing = int(out["region"].isna().sum())
    if n_missing > 0:
        log(f"[WARN] stations missing region mapping: {n_missing} / {len(out)}")
        if require_region:
            missing_ids = out.loc[out["region"].isna(), "Station ID"].astype("Int64").tolist()
            raise RuntimeError(
                "Region lookup required but missing mappings for Station ID(s): "
                + ", ".join(str(x) for x in missing_ids if x is not None)
            )
    else:
        log(f"[INFO] region mapping complete: {len(out)} stations mapped")
...
def merge_key_duplicates(df0: pd.DataFrame, key: List[str]) -> pd.DataFrame:
    ...
    - We inject key values from g.name into the returned Series.
    """
    df = df0.copy()
    ...
    if not non_conflict_keys.empty:
        ...
        merged_non_conflict = groupby_apply_robust(gb_nc, merge_no_conflict_group).reset_index(drop=True)
    ...
    if not conflict_keys.empty:
        ...
        merged_conflict = groupby_apply_robust(gb_c, merge_conflict_group).reset_index(drop=True)
...
# -------------------------
def main() -> None:
    ap = argparse.ArgumentParser(description="Build Ontario hourly PM2.5 wide dataset (parse + station meta + de-dup).")
    ap.add_argument("--input-dir", type=str, default=str(DEFAULT_INPUT_DIR), help="Local directory of raw PM2.5 files.")
    ap.add_argument("--base-raw", type=str, default=DEFAULT_BASE_RAW, help="Remote base URL (only if local missing).")
    ap.add_argument("--files", nargs="*", default=DEFAULT_FILES, help="Filenames to process.")
    ap.add_argument("--timeout", type=int, default=60, help="Request timeout seconds (if remote fetch).")
    ap.add_argument("--out", type=str, default="outputs/pm25_hourly_wide_final.csv", help="Output CSV path.")
    ap.add_argument("--out-station-lookup", type=str, default="outputs/station_lookup.csv", help="Station lookup output.")
    ap.add_argument("--keep-year", action="store_true", help="Keep helper column 'year' in final output.")

    # Region mapping options
    ap.add_argument(
        "--station-region-lookup",
        type=str,
        default=str(DEFAULT_STATION_REGION_LOOKUP),
        help="CSV path with columns: Station ID, region (optional but recommended).",
    )
    ap.add_argument(
        "--require-region",
        action="store_true",
        help="Fail the run if region lookup is missing or incomplete.",
    )
    ...
    # Attach region mapping (optional)
    station_lookup = attach_station_region(
        station_lookup=station_lookup,
        lookup_path=Path(args.station_region_lookup),
        template_out=Path("outputs/station_region_lookup_TEMPLATE.csv"),
        require_region=bool(args.require_region),
    )
    ...
    # Merge station meta + region
    if station_lookup.empty:
        log("[WARN] station_lookup is empty -> output will not have station_name/lat/lon/region")
    ...
    meta_cols = [c for c in meta_order if c in pm25_hourly_wide_final.columns]
