...
def build_hourly_wide(
    ...
    # normalize key columns for stable de-duplication
    df0["Date"] = pd.to_datetime(df0["Date"], errors="coerce").dt.normalize()
    if "Pollutant" in df0.columns:
        df0["Pollutant"] = df0["Pollutant"].astype("string").str.strip()
    ...
    conflict_flag = (
        df_dup.groupby(key, dropna=False, group_keys=False).apply(is_conflict_group).reset_index(name="is_conflict")
    )
    ...
    merged_non_conflict = (
        df_dup_non_conflict.groupby(key, dropna=False, group_keys=False).apply(merge_no_conflict_group).reset_index(drop=True)
    )
    ...
    merged_conflict = (
        df_dup_conflict.groupby(key, dropna=False, group_keys=False).apply(merge_conflict_group).reset_index(drop=True)
    )
    ...
    if out.duplicated(subset=key, keep=False).any():
        n_dup_rows = int(out.duplicated(subset=key, keep=False).sum())
        print(f"[WARN] duplicate keys remain after first-pass merge (rows={n_dup_rows}). Running second-pass merge...")
        out = (
            out.groupby(key, dropna=False, group_keys=False)
            .apply(merge_conflict_group)
            .reset_index(drop=True)
        )

    if out.duplicated(subset=key, keep=False).any():
        dups = out.loc[out.duplicated(subset=key, keep=False), key].drop_duplicates()
        example = dups.head(10).to_string(index=False)
        raise RuntimeError(
            "De-duplication failed: duplicate keys still exist after second-pass merge. "
            f"n_keys={len(dups)}. Example keys:\n{example}"
        )
