...
def main():
    ...
    # ---- Temperature (limit=5, forward only) ----
    temp_cols = [
    ...
    for col in temp_cols:
        if col in df.columns:
            ...
            print(f"[IMPUTE] {col} -> forward linear interpolate (limit=5)")

    # ---- Wind Speed (limit=3, forward only) ----
    ...
    for col in wind_speed_cols:
        if col in df.columns:
            ...
            print(f"[IMPUTE] {col} -> forward linear interpolate (limit=3)")

    # ---- Wind Direction (ffill limit=2) ----
